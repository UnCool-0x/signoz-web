...
<Tabs>
<TabItem value="direct-to-signoz" label="Direct to SigNoz" default>
...
Add this function in main.rs file, `init_tracer` is initializing an OpenTelemetry tracer with the OpenTelemetry OTLP exporter which is sending data to SigNoz. 
...
add the following block to send data to SigNoz
...
Run `cargo run` command in terminal to start the application.

</TabItem>
<TabItem value="via-otel-collector" label="via Otel Collector">
...
Run the following command to start Rust app.

```
OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317 OTEL_RESOURCE_ATTRIBUTES=service.name=<service_name> cargo run
```
